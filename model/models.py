...
class CompGCNBase(BaseModel):
	def __init__(self, curv, edge_index, edge_type, num_rel, params=None):
		...
		if self.p.num_bases > 0:
			self.conv1 = CompGCNConvBasis(self.curv, self.p.init_dim, self.p.gcn_dim, num_rel, self.p.num_bases, act=self.act, params=self.p)
			self.conv2 = CompGCNConv(self.curv, self.p.gcn_dim,    self.p.embed_dim,    num_rel, act=self.act, params=self.p) if self.p.gcn_layer == 2 else None
		else:
			self.conv1 = CompGCNConv(self.curv, self.p.init_dim, self.p.gcn_dim,      num_rel, act=self.act, params=self.p)
			self.conv2 = CompGCNConv(self.curv, self.p.gcn_dim,    self.p.embed_dim,    num_rel, act=self.act, params=self.p) if self.p.gcn_layer == 2 else None
...
class CompGCN_TransE(CompGCNBase):
	def __init__(self, curv, edge_index, edge_type, params=None):
		super(self.__class__, self).__init__(curv, edge_index, edge_type, params.num_rel, params)
