...
class CompGCNConvBasis(MessagePassing):
	def __init__(self, curv, in_channels, out_channels, num_rels, num_bases, act=lambda x:x, cache=True, params=None):
	...
	def forward(self, x, edge_index, edge_type, edge_norm=None, rel_embed=None):
		...
		self.curv = self.w_mlp_out(self.curvy)
		...
		self.curv = softmax(self.curv, self.in_index[0])
	...
	def message(self, x_j, curv, edge_type, rel_embed, edge_norm, mode, edge_index):
		...
		curvlist=torch.Tensor(curv)
		out = torch.multiply(out,curvlist)
